...
class ReceiptCompletedScreen extends StatefulWidget {
  @override
  _ReceiptCompletedScreenState createState() => _ReceiptCompletedScreenState();
}

class _ReceiptCompletedScreenState extends State<ReceiptCompletedScreen> {
  String currentEmail = '';
  ...
  //삭제되면 delete 업로드
  Future<List<Map<String, dynamic>>> _fetchCompletedReceipts() async {
    try {
      // Firestore에서 completedReceipts 컬렉션에서 ownerEmail이 현재 이메일과 일치하는 문서들을 가져옴
      QuerySnapshot snapshot = await FirebaseFirestore.instance
          .collection('completedReceipts')
      ...
      return snapshot.docs
        ...
        return data != null && (data.containsKey('delete') ? data['delete'] != true : true);
      })
  ...
  //수령 내역 삭제
  Future<void> _deleteReceipt(String docName) async{
    try {
        ...
        builder: (context, snapshot) {
                ...
                Container(
                  ...
                  child: RichText(
                    text: TextSpan(
                      ...
                      children: [
                        TextSpan(
                          text: '${receipts.length}건', // receipts.length를 이용해 건수 표시
                          style: TextStyle(
                            fontFamily: 'Pretendard',
                            fontWeight: FontWeight.w700,
                            fontSize: 20,
          ...
          child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      Map<String, dynamic> receipt = receipts[index];
                      return
                        _buildPostCard(
                          docName: receipt['docName'],
                          timeAgo: DateFormat('yyyy-MM-dd HH:mm').format(receipt['timeAgo'].toDate()),
                          storeName: receipt['storeName'] ?? '가게 이름 없음',
                          location: receipt['location'] ?? '위치 정보 없음',
                          cost: receipt['cost'] ?? '비용 정보 없음',
                          helper_email_nickname: receipt['helperNickname'] ?? '사용자 이름 없음',
                          helperPhotoUrl: receipt['helperPhotoUrl'] ?? '사용자 사진 없음',
                        );
                    },
                  ),
                  ),
